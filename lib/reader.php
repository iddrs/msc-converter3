...
function load_source_csv(string $filePath): array
{
    if (!file_exists($filePath))
        trigger_error("Arquivo $filePath não encontrado.", E_USER_ERROR);

    $data = file($filePath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
    if ($data === false)
        trigger_error("Não foi possível ler dados de $filePath");
...
function parse_remessa_to_filepath(string $mscOutputBaseDir, int $remessa): string
{
    ...
    return sprintf("%s\%s\MSC_MES_%s.csv", $mscOutputBaseDir, $ano, $mes);//MSC_MES_8.csv
}